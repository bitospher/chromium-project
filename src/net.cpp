...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 1; nHost++)
    {
        ...
        if (nHost == 1)
        {
            addrConnect = CService("138.197.78.227", 80); // checkip.dyndns.org
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Chromium " + FormatFullVersion();
...
static const char *strTestNetDNSSeed[][2] = {
    {"chromiumtools.com", "testnet-seed.chromiumtools.com"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Chromium is probably already running."), addrBind.ToString().c_str());
        else
