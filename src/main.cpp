...
#if defined(NDEBUG)
# error "Chromium cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5651ea7756434dccf7645815ee1f3c6261cfdc769ac54b4657035b056419c219");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Chromium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Chromium Signed Message:\n";
...
{
    // Chromium: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Chromium: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Chromium: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Chromium: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Chromium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Chromium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5651ea7756434dccf7645815ee1f3c6261cfdc769ac54b4657035b056419c219");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Chromium Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xe4e34d9edc970aa513db4fd38ebfaa5b985106c4293e6a38705397fb4a6ac858"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Chromium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChromiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChromiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChromiumMiner(CWallet *pwallet)
{
    printf("ChromiumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chromium-miner");
    ...
    try { loop {
        ...
        printf("Running ChromiumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChromiumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChromiumMiner, pwallet));
}
